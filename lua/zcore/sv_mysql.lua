...
local function connect()
	...
	ServerLog("[ZCore] Connecting to MySQL on " .. GetConVarString("sv_zcore_mysql_hostname") .. ":3306...\n")
	database = mysqloo.connect(GetConVarString("sv_zcore_mysql_hostname"), GetConVarString("sv_zcore_mysql_username"), GetConVarString("sv_zcore_mysql_password"), GetConVarString("sv_zcore_mysql_database"))
...
hook.Add("PlayerInitialSpawn", "ZCoreSQLConnect", function()
    if string.len(GetConVarString("sv_zcore_mysql_hostname")) > 0 then
        connect()
    else
        ServerLog("[ZCore] Please configure MySQL cvars to use ... uh ... MySQL!\n")
    end
end)
